...
module.exports = {
  ...
  run: async function (client, message, args) {
   const queue = message.client.queue.get(message.guild.id);
    ...
    if (!args.length) return sendError(`Usage: ${process.env.PREFIX}\`remove <Queue Number>\``);
    if (isNaN(args[0])) return sendError(`Usage: ${process.env.PREFIX}\`remove <Queue Number>\``);
